...
onde coluns $x_i$ são os autovetores. Como os autovetores são ortogonais, normalizando cada $x_i$, temos
$$ S = Q\Lambda Q^{-1}$$

Como $Q$ é quadrada (e ortogonal), $$Q^{-1} = Q\Lambda Q^\top$$

## Matrizes definidas positivas {.scrollable}

1. Todos os $\lambda_i >0$
2. Energia $x^\top S x>0$ $\forall x>0$
...
:::{.callout-warning}
Definição de matrizes similares: $A = MBM^{-1} = P^{-1}BP$
:::
...
## Matrizes simidefinidas positivas
